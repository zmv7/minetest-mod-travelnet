...
-- minetest.chat_send_player(puncher_name, "data: "..minetest.serialize(  travelnet.targets ));
   ...
   if(  not( travelnet.targets[ owner_name ] )
     ...
     or not( travelnet.targets[ owner_name ][ station_network ][ station_name ] )) then
      ...
      minetest.chat_send_player(owner_name, S("Station '@1'", station_name).." "..
		S(" has been reattached to the network '@1'.", station_network));
      travelnet.save_data();
   end
...
--         formspec = formspec .. ']';
   ...
   meta:set_string( "infotext", S("Station '@1' on travelnet '@2' (owned by @3) ready for usage. Right-click to travel, punch to update.", tostring(station_name), tostring(station_network), tostring(owner_name)))
...
--      owner_name   = '*'; -- the owner name is not relevant here
   ...
   if( anz+1 > travelnet.MAX_STATIONS_PER_NETWORK ) then
      travelnet.show_message( pos, player_name, S("Error"),
	S("Network '@1',", network_name).." "..
	S("already contains the maximum number (=@1) of allowed stations per network. "..
	"Please choose a diffrent/new network name.", travelnet.MAX_STATIONS_PER_NETWORK));
      return;
   end
   ...
   if( meta ) then
      ...
      minetest.chat_send_player(player_name, S("Station '@1' has been added to the network '@2', which now consists of @3 station(s).", station_name, network_name, anz+1))
...
-- TODO: actually add help page
   ...
   if( fields.station_dig ) then
      ...
      if( not(minetest.check_player_privs(name, {travelnet_remove=true}))
       ...
       and owner ~= "") then
         minetest.chat_send_player(name, S("This @1 belongs to @2. You can't remove it.", description, meta:get_string('owner')));
         return
      end
   ...
   if( not( travelnet.targets[ owner_name ][ station_network ][ fields.target ] )) then
      ...
      minetest.chat_send_player(name, S("Station '@1'", fields.target or "?").." "..
...
travelnet.remove_box = function( pos, oldnode, oldmetadata, digger )
   ...
   -- inform the owner
   local msg = S("Station '@1' has been REMOVED from the network '@2'", station_name, station_network)
   minetest.chat_send_player( owner_name, msg);
